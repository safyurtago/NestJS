...
function password_derive_bytes(password, salt, iterations, len) {
  ...
  for (let i = 0; i < iterations; i++) key = sha256(key);
  
  if (key.length < len) {
    const hx = password_derive_bytes(password, salt, iterations - 1, 20);

    for (let counter = 1; key.length < len; ++counter)  key = Buffer.concat([ key, sha256(Buffer.concat([Buffer.from(counter.toString()), hx])) ]);
  }
